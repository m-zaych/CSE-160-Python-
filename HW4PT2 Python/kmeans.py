...
def euclidean_distance(point1, point2):
    ...
    for i in range(len(point1)):
        euc_totals = euc_totals + ((point1[i] - point2[i]) ** 2)
    return math.sqrt(euc_totals)
...
def get_closest_centroid(point, centroids_dict):
    ...
    # REMOVE THIS COMMENT AND REPLACE IT WITH YOUR CODE ...
    # distance_near = 100000000000000000000000000 * 10000000000000000
    distance_near = math.inf
    nearest_centroid = ""
    for i in list(centroids_dict.keys()):
        distance_centroids = euclidean_distance(point, centroids_dict[i])
        if distance_centroids <= distance_near:
            distance_near = distance_centroids
            nearest_centroid = i
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for point in list_of_points:
        what_centroid = get_closest_centroid(point, centroids_dict)
        if what_centroid not in new_key_centroid_names_dict:
            new_key_centroid_names_dict[what_centroid] = []
        new_key_centroid_names_dict[what_centroid].append(point)
...
def mean_of_points(list_of_points):
    ...
    for i in range(len(list_of_points[0])):
        ...
        for j in range(len(list_of_points)):
            total = total + list_of_points[j][i]
            num = num + 1
        cumulative = total / num
        average_val_list.append(cumulative)
    return average_val_list
...
def update_centroids(assignment_dict):
    ...
    # REMOVE THIS COMMENT AND REPLACE IT WITH YOUR CODE ...
    new_dictionary = {}
    for keyvals, lists in assignment_dict.items():
        new_dictionary[keyvals] = mean_of_points(lists)
