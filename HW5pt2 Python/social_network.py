...
def influence_map(graph, user):
    ...
    for homies in friends_of_friends(graph, user):
        solo_results = 0
...
def get_facebook_graph():
    ...
    facebook_constructor = nx.Graph()
    facebook_reader = ('facebook-links-small.txt')
    myfile = open(facebook_reader)
    for text in myfile:
        storage_map = list(text.split())
        facebook_constructor.add_edge(int(storage_map[0]), int(storage_map[1]))
    myfile.close()
    return facebook_constructor
...
def main():
    ...
    for homies in sorted(facebook.nodes()):
        if homies % 1000 == 0:
            homies_tallied_rncf = rec_number_common_friends(facebook, homies)
            print(homies, "(by num of common friends):",
                  (homies_tallied_rncf[0:10]))
    ...
    for homies in sorted(facebook.nodes()):
        if homies % 1000 == 0:
            homies_tallied_rbi = (recommend_by_influence(facebook, homies))
            print(homies, "(by influence):", (homies_tallied_rbi[0:10]))
    ###
    #  Problem 8
    ###
    print()
    ...
    for homies in sorted(facebook.nodes()):
        if homies % 1000 == 0:
            rbi_list = (recommend_by_influence(facebook, homies))
            rncf_list = (rec_number_common_friends(facebook, homies))
            if rncf_list[0:10] != rbi_list[0:10]:
                different_val_ctr = different_val_ctr + 1
            else:
                same_val_ctr = same_val_ctr + 1
    print("Same:", same_val_ctr)
    print("Different:", different_val_ctr)
