...
# collaborated with Parviz Sadikov
# collaborated with Artyom Serebryakov
# discussed and visualized nested lists
# attended sneh's office hours went over the average_of func
# sneh also helped me understand the spec as well as lists
# attended fitz's office hours to help with the lists as well
# finally fitz's office hours went over the opening and closing of the file
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    for r in range(i-1, i+2):
        for c in range(j-1, j+2):
            pixel_sum = get_pixel_at(pixel_grid, r, c) + pixel_sum
    return pixel_sum // 9
...
def blur(pixel_grid):
    ...
    for row in range(0, len(pixel_grid)):
        empty_l = []
        for col in range(0, len(pixel_grid[0])):
            new_grid = average_of_surrounding(pixel_grid, row, col)
            empty_l.append(new_grid)
        blurry_grid.append(empty_l)
